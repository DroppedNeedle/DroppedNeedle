...
async def test_old_source_answer_cannot_overwrite_new_source_cache(monkeypatch):
    ...
    try:
        ...
        assert old_result == new_result == {"artist": []}
        assert client.urls == [
        ...
        assert (
            await mb_base.mb_cache_set_if_current(
                cache, "artist", old_result, ttl_seconds=60, context=old_context
            )
            is False
        )
        ...
        assert (
            await mb_base.mb_cache_set_if_current(
                cache, "artist", new_result, ttl_seconds=60, context=new_context
            )
            is True
        )
