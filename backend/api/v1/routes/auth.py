...
def _current_musicbrainz_source(preferences_service) -> MusicBrainzSourceIdentity:
    settings = preferences_service.get_musicbrainz_connection()
    return MusicBrainzSourceIdentity(
...
async def setup(
    ...
    auth: AuthService = Depends(get_auth_service),
    preferences_service=Depends(get_preferences_service),
) -> AuthResponse:
    ...
    return AuthResponse(
        ...
        user=user_to_response(
            user, musicbrainz_source=_current_musicbrainz_source(preferences_service)
        ),
    )
...
async def me(
    ...
    return user_to_response(
        current_user,
        providers.get(current_user.id),
        musicbrainz_source=_current_musicbrainz_source(preferences_service),
    )
...
async def list_sessions(
    ...
    return SessionListResponse(
        sessions=[session_to_response(token) for token in tokens]
    )
...
async def admin_list_users(
    ...
    providers_by_user = await auth.get_provider_names_for_users(
        [user.id for user in users]
    )
    return UserListResponse(
        users=[
            user_to_response(user, providers_by_user.get(user.id)) for user in users
        ],
        total=total,
    )
...
async def admin_create_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Could not create user"
        )
...
async def admin_import_users(
    ...
    if body.provider not in ("jellyfin", "plex"):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Unsupported import provider",
        )
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Could not import users"
        )
...
async def admin_set_role(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Could not update role"
        )
